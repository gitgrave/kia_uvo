...
class HyundaiKiaConnectDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def async_check_and_refresh_token(self):
        ...
        try:
            ...
            # Log at WARNING rather than ERROR: these are transient Kia EU API
            # failures (DeviceIDError, ReadTimeoutError, …) that typically
            # resolve on the next poll without any user action required.
            _LOGGER.warning(
                "Token refresh encountered a transient error (will retry): %s", err
            )
            raise
